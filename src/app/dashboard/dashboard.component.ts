...
@Component({
  ...
  template: `
    <div class="app">
      ...
      <main class="main">
        ...
        <!-- Pie Chart -->
        <section class="chart-section" *ngIf="allStats.length > 0">
          <div class="chart-card">
            <div class="chart-header">
              <h3>Spending by Category</h3>
              <a routerLink="/reports" class="view-all">View Full Report →</a>
            </div>
            <div class="chart-body">
              ...
              <div class="legend">
                <div class="legend-item" *ngFor="let stat of allStats; let i=index">
                  <span class="ldot" [style.background]="colors[i % colors.length]"></span>
                  <span class="lname">{{ stat._id }}</span>
                  <span class="lval">₹{{ stat.total | number:'1.0-0' }}</span>
                  <span class="lpct">{{ getPercent(stat.total) }}%</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <section class="expense-list">
          ...
          <div class="expense-item" *ngFor="let e of expenses">
            <div class="expense-left">
              <div class="expense-cat-badge">{{ getCatIcon(e.category) }}</div>
            ...
            <div class="expense-right">
              ...
              <div class="expense-actions">
                <button class="action-btn edit" (click)="editExpense(e._id)">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                </button>
                <button class="action-btn delete" (click)="deleteExpense(e._id)">
  ...
  styles: [`
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sidebar { width: 220px; background: #1a1a1a; color: #fff; display: flex; flex-direction: column; padding: 24px 0; position: fixed; height: 100vh; left: 0; top: 0; }
    ...
    .btn-add { display: flex; align-items: center; gap: 6px; background: #1a1a1a; color: #fff; border: none; padding: 10px 18px; border-radius: 4px; font-size: 13px; font-weight: 600; cursor: pointer; text-decoration: none; }
    ...
    .chart-header h3 { font-size: 16px; font-weight: 700; color: #1a1a1a; }
    .view-all { font-size: 13px; color: #666; text-decoration: none; font-weight: 600; }
    .view-all:hover { color: #1a1a1a; }
    .chart-body { display: grid; grid-template-columns: 260px 1fr; gap: 32px; align-items: center; }
    .chart-wrap { height: 240px; position: relative; }
    .legend { display: flex; flex-direction: column; gap: 10px; }
    .legend-item { display: flex; align-items: center; gap: 8px; font-size: 13px; }
    .ldot { width: 12px; height: 12px; border-radius: 50%; flex-shrink: 0; }
    .lname { flex: 1; color: #555; font-weight: 500; }
    .lval { font-weight: 700; color: #1a1a1a; }
    .lpct { font-size: 12px; color: #aaa; }
    .filters { background: #fff; border-radius: 6px; padding: 16px 20px; display: flex; gap: 16px; align-items: flex-end; margin-bottom: 20px; box-shadow: 0 1px 4px rgba(0,0,0,0.05); flex-wrap: wrap; }
    .filter-group { display: flex; flex-direction: column; gap: 5px; }
    .filter-group label { font-size: 11px; font-weight: 600; color: #888; text-transform: uppercase; }
    .filter-group select, .filter-group input { border: 1.5px solid #e0e0e0; border-radius: 4px; padding: 8px 12px; font-size: 13px; outline: none; background: #fff; }
    .btn-clear { background: #f0f0ee; border: none; padding: 8px 16px; border-radius: 4px; font-size: 13px; cursor: pointer; color: #666; }
    .expense-list { background: #fff; border-radius: 6px; box-shadow: 0 1px 4px rgba(0,0,0,0.05); overflow: hidden; }
    .list-header { display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; border-bottom: 1px solid #f0f0ee; font-size: 14px; font-weight: 600; color: #1a1a1a; }
    ...
    .expense-item { display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; border-bottom: 1px solid #f8f8f6; transition: background 0.1s; }
    .expense-item:hover { background: #fafaf8; }
    .expense-item:last-child { border-bottom: none; }
    .expense-left { display: flex; align-items: center; gap: 14px; }
    .expense-cat-badge { width: 38px; height: 38px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 17px; background: #f5f5f0; flex-shrink: 0; }
    ...
    .action-btn { width: 28px; height: 28px; border: 1.5px solid #e8e8e8; border-radius: 4px; background: #fff; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.15s; color: #888; }
    .action-btn.edit:hover { border-color: #1a1a1a; color: #1a1a1a; }
    .action-btn.delete:hover { border-color: #e53e3e; color: #e53e3e; }
    @media (max-width: 768px) { .sidebar { display: none; } .main { margin-left: 0; padding: 16px; } .summary-grid { grid-template-columns: 1fr 1fr; } .chart-body { grid-template-columns: 1fr; } .filters { flex-direction: column; } }
  `]
})
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  allStats: { _id: string; total: number; count: number }[] = [];
  ...
  constructor(private expenseService: ExpenseService, private auth: AuthService, private router: Router) {}
  ...
  loadExpenses() {
    this.loading = true;
    this.expenseService.getExpenses({ category: this.selectedCategory, startDate: this.startDate, endDate: this.endDate }).subscribe({
      next: (res) => { this.expenses = res.expenses; this.total = res.total; this.loading = false; },
  ...
  loadStats() {
    this.expenseService.getStats().subscribe(stats => {
      ...
      this.allStats = stats;
      this.grandTotal = stats.reduce((s, i) => s + i.total, 0);
      setTimeout(() => this.renderPie(), 500);
    });
  }

  renderPie() {
    if (typeof Chart === 'undefined' || !this.pieChartRef || this.allStats.length === 0) {
      setTimeout(() => this.renderPie(), 500);
      return;
    }
    ...
    this.pieChartInstance = new Chart(this.pieChartRef.nativeElement.getContext('2d'), {
      type: 'doughnut',
      data: {
        labels: this.allStats.map(s => s._id),
        datasets: [{ data: this.allStats.map(s => s.total), backgroundColor: this.colors, borderWidth: 3, borderColor: '#fff', hoverOffset: 8 }]
      },
      options: {
  ...
  getPercent(total: number): string {
    ...
    return ((total / this.grandTotal) * 100).toFixed(1);
  }

  applyFilters() { this.loadExpenses(); }
  clearFilters() { this.selectedCategory = 'All'; this.startDate = ''; this.endDate = ''; this.loadExpenses(); }
  editExpense(id: string) { this.router.navigate(['/edit-expense', id]); }
  deleteExpense(id: string) {
    if (!confirm('Delete this expense?')) return;
    this.expenseService.deleteExpense(id).subscribe(() => { this.loadExpenses(); this.loadStats(); });
  }
  getCatIcon(cat: string): string {
    const icons: Record<string, string> = { Food: '🍽️', Transport: '🚗', Shopping: '🛍️', Bills: '📄', Health: '💊', Entertainment: '🎬', Education: '📚', Other: '📦' };
    return icons[cat] || '📦';
  }

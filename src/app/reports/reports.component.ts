...
@Component({
    ...
    <div class="reports-page">
      ...
      <main class="main">
        ...
        <div class="summary-grid">
          <div class="summary-card accent">
            ...
            <div class="card-content">
              ...
              <div class="card-sub">{{ viewMode === 'monthly' ? selectedMonthName + ' ' + selectedYear : selectedYear }}</div>
            </div>
          </div>
          <div class="summary-card">
            ...
            <div class="card-content">
              ...
              <div class="card-sub">Total entries</div>
            </div>
          </div>
          <div class="summary-card">
            <div class="card-icon">📈</div>
            <div class="card-content">
              ...
              <div class="card-sub">Daily average</div>
            </div>
          </div>
          <div class="summary-card">
            <div class="card-icon">🏆</div>
            <div class="card-content">
              ...
              <div class="card-value top-cat">{{ topCategory }}</div>
        ...
        <div class="charts-grid">
          ...
          <div class="chart-card">
            ...
            <div class="legend">
              <div class="legend-item" *ngFor="let item of categoryData; let i = index">
                ...
                <span class="legend-value">₹{{ item.total | number:'1.0-0' }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="chart-card">
            <div class="chart-header">
              <h3>{{ viewMode === 'monthly' ? 'Daily Spending' : 'Monthly Spending' }}</h3>
            ...
            <table>
              ...
              <tbody>
                <tr *ngFor="let item of categoryData; let i = index">
                  <td>
                    <span class="cat-dot" [style.background]="chartColors[i % chartColors.length]"></span>
                    {{ item.category }}
                  </td>
                  <td>{{ item.count }}</td>
                  <td class="amount">₹{{ item.total | number:'1.2-2' }}</td>
                  <td class="percent">{{ getPercent(item.total) }}%</td>
                  <td>
                    <div class="bar-cell">
                      <div class="bar-fill" [style.width]="getPercent(item.total) + '%'" [style.background]="chartColors[i % chartColors.length]"></div>
                    </div>
                  </td>
                </tr>
              </tbody>
  ...
  styles: [`
    * { box-sizing: border-box; margin: 0; padding: 0; }
    .reports-page { display: flex; min-height: 100vh; background: #f5f5f0; font-family: 'Segoe UI', sans-serif; }
    ...
    .chart-badge {
      background: #f0f0ee; color: #666; padding: 3px 10px;
    ...
    .legend-value { font-weight: 600; color: #1a1a1a; }
    ...
    .bar-cell { background: #f0f0ee; border-radius: 4px; height: 8px; width: 120px; overflow: hidden; }
    .bar-fill { height: 100%; background: #1a1a1a; border-radius: 4px; transition: width 0.6s ease; }
...
export class ReportsComponent implements OnInit, AfterViewInit {
  ...
  loadData() {
    ...
    this.expenseService.getExpenses({ startDate, endDate }).subscribe(res => {
      ...
      const days = this.viewMode === 'monthly'
      ...
      if (this.viewMode === 'yearly') {
        ...
        this.monthlyBreakdown = this.months.map((month, i) => ({
          month,
          total: monthMap[i]?.total || 0,
  ...
  renderPieChart() {
    ...
    this.pieChartInstance = new Chart(ctx, {
      type: 'doughnut',
      data: {
        labels: this.categoryData.map(c => c.category),
        datasets: [{
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (ctx: any) => `₹${ctx.raw.toLocaleString('en-IN', {minimumFractionDigits: 2})}`
            }
          }
        },
        cutout: '65%'
  ...
  getBarWidth(total: number): number {
    if (!this.maxMonthlyTotal) return 0;
